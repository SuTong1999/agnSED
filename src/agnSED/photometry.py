...
def Bolometric(sample, component=True, mcut=4, mdotcut=-12, npro=1):
    ...
    if component:
        ...
        logL_adaf = np.array([np.log10(simpson(10**lum, x=10**lognu)) for lum in logLnu_adaf])
        logL_diskcor = np.array([np.log10(simpson(10**lum, x=10**lognu)) for lum in logLnu_diskcor])
