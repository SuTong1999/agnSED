...
def Bolometric(sample, component=True, mcut=4, mdotcut=-12, npro=5):
    ...
    if component:
        ...
        logL_adaf = np.array([np.log10(simpson(10**lum, 10**lognu)) for lum in logLnu_adaf])
        logL_diskcor = np.array([np.log10(simpson(10**lum, 10**lognu)) for lum in logLnu_diskcor])
        return logL_adaf, logL_diskcor
    else:
        with Pool(processes=npro) as pool:
            logLnu = pool.map(SEDPool, data)
        logL = np.array([np.log10(simpson(10**lum, 10**lognu)) for lum in logLnu])
        return logL

def Photometric(sample, lognu0, lognu1, component=True, mcut=4, mdotcut=-12, npro=5):
    ...
    if component:
        ...
        logL_adaf = np.array([np.log10(simpson(10**logLnu, 10**lognu_band)) for logLnu in logLnu_adaf])
        logL_diskcor = np.array([np.log10(simpson(10**logLnu, 10**lognu_band)) for logLnu in logLnu_diskcor])
        return logL_adaf, logL_diskcor
    else:
        with Pool(processes=npro) as pool:
            logLnu = pool.map(SEDPool, data)
        ...
        logL = np.array([np.log10(simpson(10**lum, 10**lognu_band)) for lum in logLnu])
        return logL

def LuminosityPerHerz(sample, lognu0, component=True, mcut=4, mdotcut=-12, npro=5):
